...
def init_args():
    ...
    parser.add_argument('--student_or_teacher',default='teacher')
    
    parser.add_argument('--uda_weight', default=0.00025,type=float)
    # parser.add_argument('--uda_weight', default=0.002,type=float)
...
def train_adan_student(args, train_dataset,train_dataset_source_without_label,train_dataset_target_without_label,train_dataset_s2t_without_label,train_dataset_t2s_without_label, model, tokenizer):
    """ Train the model """
...
def train_adan_xlmr_student(args, train_dataset,train_dataset_source_without_label,train_dataset_target_without_label,train_dataset_s2t_without_label,train_dataset_t2s_without_label, model, tokenizer):
    """ Train the model """
...
def train_adan_xlmr(args, train_dataset,train_dataset_source_without_label,train_dataset_target_without_label,train_dataset_s2t_without_label,train_dataset_t2s_without_label, model, tokenizer):
    """ Train the model """
...
def train_adan_con_xlmr_kd(args, train_dataset, student_model_F, student_model_P,tokenizer):
    """ Train the model """
    ...
    # Q = XLMABSALanguageDetector.from_pretrained(args.model_name_or_path,config=config2)
    F, P = F.to(args.device), P.to(args.device)
    optimizer = optim.Adam(list(F.parameters()) + list(P.parameters()),
                           lr=args.learning_rate, eps=args.adam_epsilon)
    #optimizerQ = optim.Adam(Q.parameters(), lr=args.learning_rate)
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total)
...
def train_adan_con_mbert_kd(args, train_dataset, student_model_F, student_model_P,tokenizer):
    """ Train the model """
...
def get_teacher_probs_MSMO(args, dataset, model_class1,model_class2, teacher_F_model_path,teacher_P_model_path):
    teacher_model_F = model_class1.from_pretrained(teacher_F_model_path)
    teacher_model_F.to(args.device)

    teacher_model_P = model_class2.from_pretrained(teacher_P_model_path)
    teacher_model_P.to(args.device)
    ...
    # compute logits for the dataset using the model!
    logger.info(f"***** Compute logits for [{args.tgt_lang}] using the model {teacher_F_model_path} *****")
    logger.info("  Num examples = %d", len(dataset))
    logger.info("  Batch size = %d", args.eval_batch_size)

    preds = None
    teacher_model_F.eval()
    teacher_model_P.eval()
    ...
    for batch in eval_dataloader:
        with torch.no_grad():
            ...
            outputs_F = teacher_model_F(**inputs_no_labels)
            # outputs = model(**inputs)
            outputs = teacher_model_P(outputs_F,**inputs)
...
def get_multi_teacher_probs_MSMO(args, dataset, model_class1,model_class2):
    teacher_F_paths = args.trained_teacher_F_paths 
    teacher_P_paths = args.trained_teacher_P_paths 
   
    
    # obtain all preds
    # all_preds = []
    # for teacher_path in teacher_paths:
    #     preds = get_teacher_probs_MSMO(args, dataset, model_class1,model_class2, teacher_F_model_path,teacher_P_model_path)
    #     all_preds.append(preds)
    # obtain all preds
    all_preds = []
    for teacher_F_path in teacher_F_paths:
        F_step = get_step_from_path(teacher_F_path)
        for teacher_P_path in teacher_P_paths:
            P_step = get_step_from_path(teacher_P_path)
            if F_step == P_step:
                preds = get_teacher_probs_MSMO(args, dataset, model_class1, model_class2, teacher_F_path, teacher_P_path)
                all_preds.append(preds)
                break  # If match is found, no need to check further P paths
...
def main():
    ...
    if args.do_train:
        ...
        if args.student_or_teacher=='student':
            _, _ = train_adan_student(args, train_dataset,train_dataset_source_without_label,train_dataset_target_without_label,train_dataset_s2t_without_label,train_dataset_t2s_without_label, model, tokenizer)
        # _, _ = train_adan_xlmr_student(args, train_dataset,train_dataset_source_without_label,train_dataset_target_without_label,train_dataset_s2t_without_label,train_dataset_t2s_without_label,train_dataset_source_with_label,train_dataset_target_with_label,train_dataset_alignid_source,train_dataset_alignid_target, model, tokenizer)
    ...
    if args.do_distill:
        ...
        tokenizer1 = tokenizer_class1.from_pretrained(args.model_name_or_path)
        tokenizer2 = tokenizer_class2.from_pretrained(args.model_name_or_path)

        if args.exp_type == 'macs_kd':
            dataset = build_or_load_dataset(args, tokenizer1, mode='unlabeled_mtl')
        else:
            dataset = build_or_load_dataset(args, tokenizer1, mode='unlabeled')
        
        # teacher_model_path = get_teacher_model_path(args)
        if args.exp_type == 'acs_kd_s':
            #teacher_model_path = args.trained_teacher_paths
            teacher_F_model_path = args.trained_teacher_F_paths
            teacher_P_model_path = args.trained_teacher_P_paths
          
            # teacher_probs = get_teacher_probs(args, dataset, model_class, teacher_model_path)
            teacher_probs = get_teacher_probs_MSMO(args, dataset, model_class1, model_class2, teacher_F_model_path,teacher_P_model_path)
        ...
        # use the three combinations as multi-teacher distill
            ...
            # teacher_probs = get_teacher_probs_MSMO(args, dataset, model_class1, model_class2, teacher_model_path)
            teacher_probs = get_multi_teacher_probs_MSMO(args, dataset, model_class1, model_class2)
        ...
        # train_dataset_source = build_or_load_dataset_languagedetect_source(args, tokenizer1, mode='train')
        # train_dataset_target = build_or_load_dataset_languagedetect_target(args, tokenizer1, mode='train')
        # train_dataset_source2target = build_or_load_dataset_languagedetect_source2target_codeswutch(args, tokenizer1, mode='train')
        # train_dataset_target2source = build_or_load_dataset_languagedetect_target2source_codeswitch(args, tokenizer1, mode='train')
        # train_dataset_source_with_label=build_or_load_dataset_languagedetect_source_label(args, tokenizer1, mode='train')
        # train_dataset_target_with_label=build_or_load_dataset_languagedetect_target_label(args, tokenizer1, mode='train')
        
        # train_dataset_alignid_source = build_or_load_dataset_alignid_source(args, tokenizer1, mode='train')
        # train_dataset_alignid_target = build_or_load_dataset_alignid_target(args, tokenizer1, mode='train')
        # # train_dataset_source_without_label = train_dataset_source.encodings
        # # train_dataset_target_without_label = train_dataset_target.encodings
        # # train_dataset_s2t_without_label = train_dataset_source2target.encodings
        # # train_dataset_t2s_without_label = train_dataset_target2source.encodings
        ...
        #student_model_path = f"/data/wuchengyan/XABSA-master/XLM-R"

        #student_model_path = args.student_model_path

        # s_config_class, s_model_class, _ = MODEL_CLASSES[args.tfm_type]
        s_config_class1, s_model_class1, _ = MODEL_CLASSES[args.featuremodel]
       
        config1 = s_config_class1.from_pretrained(
            args.config_name if args.config_name else args.model_name_or_path,
            num_labels=num_tags, id2label=idx2tag, label2id=tag2idx
        )       
        s_config_class2, s_model_class2, _ = MODEL_CLASSES[args.sentimentmodel]
       
        config2 = s_config_class2.from_pretrained(
            args.config_name if args.config_name else args.model_name_or_path,
            num_labels=num_tags, id2label=idx2tag, label2id=tag2idx
        )
        #student_model_path = args.model_name_or_path   # not good
        
        student_model_F = s_model_class1.from_pretrained(args.student_F_model_path)
        student_model_P = s_model_class2.from_pretrained(args.student_P_model_path)
       
        student_model_F.to(args.device)
        student_model_P.to(args.device)
       
        # logger.info(f"We initialize the student model with {student_model_path}")
        logger.info(f"We initialize the student model with {args.student_F_model_path} and {args.student_P_model_path}")
       
        #_, _ = train_kd(args, train_dataset, student_model, tokenizer)
        # _, _ = train_adan_con_xlmr_kd(args,train_dataset, student_model_F, student_model_P,tokenizer1)
        _, _ = train_adan_con_mbert_kd(args,train_dataset, student_model_F, student_model_P, tokenizer1)
    ...
    if args.do_eval:
        ...
        if 'mtl' in exp_type:
            ...
            #saved_model_dir = f"output_dir_loadargs/{args.tfm_type}-{args.src_lang}-{one_tgt_lang}-{exp_type}"
